...
struct FFTDataGenerator
{
    ...
    void changeOrder(FFTOrder newOrder)
    {
        ...
        //things that need sizing should be sized via resize()
...
struct AnalyzerPathGenerator
{
        ...
        float negativeInfinity)
    {
        ...
        PathType p;
        p.preallocateSpace(3 * (int)renderArea.getWidth());

        auto map = [bottom, top, negativeInfinity](float v)
            {
                return juce::jmap(v, negativeInfinity, 0.f, float(bottom), float(top));
            };
        ...
        const int pathResolution = 2; // you can draw multiple points at the same x coord if you don't do this
...
struct RotarySliderWithLabels : juce::Slider {
    RotarySliderWithLabels(juce::RangedAudioParameter& rap, const juce::String& unitSuffix)
        : juce::Slider(juce::Slider::RotaryHorizontalVerticalDrag, juce::Slider::TextEntryBoxPosition::NoTextBox),
        param(&rap),
        suffix(unitSuffix)
    {
        setLookAndFeel(&lnf);
